 ...
 * Class Like
 *
 * Registers the "like" filter, an SQL-style LIKE filter.
 *
 * like(content_raw, %some-value%)
 *
    ...
    public function register(CommentFilterManager $manager)
    {
        $manager->filter(Like::FILTER_LIKE, function ($comments) {
            ...
            if (Str::isNullOrEmpty($propertyToCheck)) {
                throw new FilterException('`like` filter: $property does not accept `null` values.');
            }
            ...
            return array_filter($comments, function (CommentContract $comment) use ($propertyToCheck, $pattern) {
                ...
                return Str::isLike($pattern, $commentValue);
            });
        }, 'property, pattern');
    }
