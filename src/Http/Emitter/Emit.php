...
class Emit
{
    ...
    {
        $assetNameForStatamic = './../'.Meerkat::CODE_ADDON_NAME;
        $fileName = basename($dynamicCssName);

        Statamic::style($assetNameForStatamic, $fileName);


        Route::get('/'.Meerkat::CODE_ADDON_NAME.'/css/'.$fileName.'.css', function () use ($callback) {
           $content = $callback();
    ...
    public static function js($dynamicJsName, $callback)
    {
        ...
        Route::get('/'.Meerkat::CODE_ADDON_NAME.'/js/'.$fileName.'.js', function () use ($callback) {
            $content = $callback();
