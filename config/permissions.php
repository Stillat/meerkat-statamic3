<?php

return [

    /*
    |--------------------------------------------------------------------------
    | All Permissions User Groups
    |--------------------------------------------------------------------------
    |
    | A list user group identifiers whose members have all permissions granted.
    |
   */
    'all_permissions' => [

    ],

    /*
    |--------------------------------------------------------------------------
    | Can View Comment User Groups
    |--------------------------------------------------------------------------
    |
    | A list user group identifiers whose members can view comments.
    |
   */
    'can_view_comments' => [

    ],

    /*
    |--------------------------------------------------------------------------
    | Approval Permission User Groups
    |--------------------------------------------------------------------------
    |
    | The user groups, whose members can approve
    | comments from within the Control Panel.
    |
   */
    'can_approve_comments' => [

    ],

    /*
    |--------------------------------------------------------------------------
    | Unapproval Permission User Groups
    |--------------------------------------------------------------------------
    |
    | The user groups, whose members can unapprove
    | comments from within the Control Panel.
    |
   */
    'can_unapprove_comments' => [

    ],
    /*
    |--------------------------------------------------------------------------
    | Reply User Groups
    |--------------------------------------------------------------------------
    |
    | The user groups, whose members can reply to
    | comments from within the Control Panel.
    |
   */
    'can_reply_to_comments' => [

    ],

    /*
    |--------------------------------------------------------------------------
    | Edit User Groups
    |--------------------------------------------------------------------------
    |
    | The user groups, whose members can modify
    | comments from within the Control Panel.
    |
    */
    'can_edit_comments' => [

    ],

    /*
    |--------------------------------------------------------------------------
    | Report Spam User Groups
    |--------------------------------------------------------------------------
    |
    | The user groups, whose members can report spam
    | comments from within the Control Panel UI.
    |
    */
    'can_report_as_spam' => [

    ],

    /*
    |--------------------------------------------------------------------------
    | Report Spam User Groups
    |--------------------------------------------------------------------------
    |
    | The user groups, whose members can report ham
    | comments from within the Control Panel UI.
    |
    */
    'can_report_as_ham' => [

    ],

    /*
    |--------------------------------------------------------------------------
    | Report Spam User Groups
    |--------------------------------------------------------------------------
    |
    | The user groups, whose members can remove
    | comments from within the Control Panel.
    |
    */
    'can_remove_comments' => [

    ],

];
